 ...
 * API to make a call to host context. All APIs are called from RDP thread.
 */
struct FreeRDPHostAPI {
    void *hostContext;
    void (*setDisplayMode)(void *hostContext, sint32 bitsPerPixel, sint32 depth,
                           sint32 bytesPerLine, sint32 width, sint32 height,
                           uint8 *preallocBuffer);
    void (*updateScreen)(void *hostContext, sint32 xMin, sint32 yMin,
                         sint32 xMax, sint32 yMax);
    void (*disconnectWithReason)(void *hostContext, sint32 reason);
};

/*
 * FreeRDP extended API for mobile platform
 */
FREERDP_API void freerdp_ext_register_host_api(struct FreeRDPHostAPI *hostApi);
FREERDP_API void freerdp_ext_disconnect(freerdp *session);
FREERDP_API freerdp* freerdp_ext_connect(const char *host, const char *username,
                                         const char *pass, const char *domain,
                                         int width, int height);
FREERDP_API void freerdp_ext_send_mouse_event(freerdp *session,
                                              int cursorx, int cursory,
                                              int button0, int button1,
                                              int button2, int button3,
                                              int button4, uint8 down);
FREERDP_API void freerdp_ext_send_key_event(freerdp *session,
