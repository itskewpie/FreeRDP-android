...
{
#ifdef LOAD_STATIC_PLUGIN
	char rename[255];
	void* entry;

        bzero(rename, 255);
        snprintf(rename, sizeof(rename), "%s_%s", name, entry_name);
        entry = DLSYM(RTLD_DEFAULT, rename);
	if (entry == NULL) {
            printf("freerdp_load_plugin: failed to load %s@%s: %s\n", rename, name, DLERROR());
            return NULL;
	}
#else
	char* path;
